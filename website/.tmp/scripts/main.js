...
*/
var CONTENTS_FOLDER = 'content/';
var PLAYLIST_FILENAME = '/default.m3u8';
var INDEX_FILENAME = 'index.json';
var LOOP_INTERVAL = 10000; // 10 seconds
...
function doPollingLoopIteration() {
  var contentZero = contents.length > 0 ? contents[0] : null;

  loadContentsIndex(function (err, data) {
    ...
    if (err) {
      ...
      var latestContentId = contents[0];
      ...
      if (autoPlay && playingContentId !== latestContentId) {
        checkIfContentFileExists(latestContentId, function (err, data) {
          if (err) {
...
function setupVideo(callback) {
  ...
  if (Hls.isSupported()) {
    ...
    hls.on(Hls.Events.ERROR, function (event, data) {
      ...
      var errorText = 'ERROR: type = ' + errorType + ', details = ' + errorDetails + reason;
...
function loadVideo(contentId) {
  ...
  var url = CONTENTS_FOLDER + contentId + PLAYLIST_FILENAME;
  console.log('URL = ' + url);
  var video = document.getElementById('video');
  video.pause();

  setupVideo(function (err, data) {
    if (err) {
      console.error(err);
      showError(err);
      return;
...
function updateContentsIndex() {
  ...
  contents.forEach(function (filename) {
    linkContents += '<a href="javascript:autoPlay = false; loadVideo(\'' + filename + '\');">' + filename + '</a><br/>';
  });
...
function loadContentsIndex(callback) {
  ...
  var key = CONTENTS_FOLDER + INDEX_FILENAME;

  loadFileFromS3(key, function (err, data) {
  ...
  ajaxRequest.onreadystatechange = function () {
    if (ajaxRequest.readyState === XMLHttpRequest.DONE) {
      if (ajaxRequest.status === 200) {
        ...
        var err = 'ERROR: Failed to load file (' + url + ') : ' + ajaxRequest.status + ' ' + ajaxRequest.statusText;
        callback(err, null);
      }
    }
  };
